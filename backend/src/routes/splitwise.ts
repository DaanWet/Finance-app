...
router.get('/connect', async (_req: Request, res: Response) => {
  ...
  try {
    ...
    res.json({ id: user.id, name: `${user.first_name} ${user.last_name}` });
  } catch (e: unknown) {
...
router.get('/expenses', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const data = await splitwiseFetch(`/get_expenses?${params}`, apiKey) as { expenses: unknown[] };
    ...
    const processed = (data.expenses as Array<{
      ...
      users: Array<{ user_id: number; owed_share: string; paid_share: string }>;
