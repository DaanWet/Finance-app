...
router.post('/', (req: Request, res: Response) => {
  ...
  const result = getDb().prepare('INSERT INTO categories (name, color, icon) VALUES (?, ?, ?)').run(name, color ?? '#94a3b8', icon ?? null);
  res.status(201).json(getDb().prepare('SELECT * FROM categories WHERE id = ?').get(result.lastInsertRowid));
});

router.put('/:id', (req: Request, res: Response) => {
  const { name, color, icon } = req.body;
  const result = getDb().prepare('UPDATE categories SET name = COALESCE(?, name), color = COALESCE(?, color), icon = COALESCE(?, icon) WHERE id = ?').run(name ?? null, color ?? null, icon ?? null, Number(req.params['id']));
